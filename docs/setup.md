...
- Página de detalhes de personagem em estilo Community Tibia com informações, mortes e varredura de frags/assistências nas deathlists de jogadores do mundo acima do level 8 via highscores.
...
A integração fica centralizada em `src/js/tibiaApi.js`, que expõe `getWorldOnline(worldName)`, `getHighscores(worldName)`, `getCharacter(characterName)` e normalizadores para manter a UI desacoplada do formato bruto da API. Na lista de online, o botão **Visualizar** abre `public/character.html` com o perfil do personagem. Essa tela também carrega os highscores de experiência do mesmo mundo para varrer deathlists de jogadores acima do level 8 e encontrar o char visualizado como autor ou assistência em mortes dos últimos 30 dias, mesmo que esses jogadores não estejam online no momento.
